...
<template>
  <div class="w-full max-w-[40rem] mx-auto flex flex-col gap-2 px-2 sm:px-0">
    <!-- Component Info -->
    <div v-if="card.title || card.text" class="space-y-2">
      <div class="flex items-center mt-4 sm:mt-6 mx-2 sm:mx-5">
    ...
    <div class="space-y-3 sm:space-y-4">
      <!-- Tab Navigation -->
      <div v-if="!isCompact" class="flex gap-1 sm:gap-2">
        <button
          @click="isPreview = true"
          :class="[
            'py-1.5 sm:py-2 px-3 sm:px-4 rounded-lg transition-all duration-200 text-xs sm:text-sm font-medium',
      ...
      >
        ...
        <button
          ...
          class="absolute top-2 sm:top-4 right-2 sm:right-4 p-1.5 sm:p-2 rounded-lg active-component-hover transition-all duration-200 z-50"
          title="Restart animation"
        >
          <Icon name="mdi:refresh" size="16" sm:size="18" class="text-p" />
        </button>
        ...
        >
          ...
          <span
            v-else
            :class="card.content?.props?.class"
            class="h-full flex items-center justify-center text-p text-xl sm:text-2xl"
          >
        ...
        <div v-else class="h-full overflow-hidden rounded-xl">
          <div
            class="flex items-center justify-between px-3 sm:px-4 py-2 sm:py-3 border-b main-border code-bg rounded-t-xl"
          >
            ...
            <button
              @click="copyCode"
              :class="[
                'flex items-center gap-1 sm:gap-2 py-1 sm:py-1.5 px-2 sm:px-3 rounded-lg transition-all duration-200 text-xs sm:text-sm font-medium',
                copied
